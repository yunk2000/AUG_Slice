...
def isControlEdgeExists(pdg, startnode, endnode):
    for edge in pdg.es:
        if pdg.vs[edge.tuple[0]]['name'] == startnode and pdg.vs[edge.tuple[1]]['name'] == endnode and edge['var'] == None:
            return True
        else:
            continue
    return False
...
def addControlEdge(pdg, startnode, endnode):
    if isControlEdgeExists(pdg, startnode, endnode):
        return pdg
...
def isNextTokenEdgeExists(pdg, startnode, endnode):
    for edge in pdg.es:
        if pdg.vs[edge.tuple[0]]['name'] == startnode and pdg.vs[edge.tuple[1]]['name'] == endnode and edge['var'] == None:
            return True
        else:
...
def addNextTokenEdge(pdg, startnode, endnode):
    if isControlEdgeExists(pdg, startnode, endnode):
        return pdg
...
def getFuncPDGById(testID, pdg_funcid, pdg_path):
    # file_dir = os.path.join("pdg_db", testID)
    file_dir = os.path.join(pdg_path, testID)
    # print "=================================="
    # print file_dir
...
def getFuncPDGByNameAndtestID_noctrl(func_name, testID):
    pdg = False
    for _dir in os.listdir("pdg_aug/"):
        list_testid = os.listdir(os.path.join("pdg_aug/", _dir))
