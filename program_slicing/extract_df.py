...
def api_slice():
    count = 1
    store_filepath = "C/test_data/api_slices.txt"
    f = open("SyVcs/sensifunc_slice_points.pkl", 'rb')
    ...
    for key in dict_unsliced_sensifunc.keys():#key is testID
        ...
        for _t in dict_unsliced_sensifunc[key]:
            ...
            if sensitive_funcname.find("main") != -1:
                ...
                pdg_path = 'pdg_aug'
                pdg = getFuncPDGById(key, pdg_funcid, pdg_path)
...
def pointers_slice():
    count = 1
    store_filepath = "C/test_data/pointersuse_slices.txt"
    f = open("SyVcs/pointuse_slice_points.pkl", 'rb')
...
def arrays_slice():
    count = 1
    store_filepath = "C/test_data/arraysuse_slices.txt"
    f = open("SyVcs/arrayuse_slice_points.pkl", 'rb')
    dict_unsliced_pointers = pickle.load(f)
    f.close()
...
def integeroverflow_slice():
    count = 1
    store_filepath = "C/test_data/integeroverflow_slices.txt"
    f = open("SyVcs/integeroverflow_slice_points.pkl", 'rb')
